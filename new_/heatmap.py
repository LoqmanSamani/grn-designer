...
class HeatMap:
    ...
    def cmap_(self):
        ...
        red_purple_ = [(0, 0, 0), ((red_[1][0] + purple_[1][0]) / 2,
                                   ...
                                   (red_[1][2] + purple_[1][2]) / 2)]  # red-purple
        ...
        # Add predefined colormaps from matplotlib
        elif self.c_map in plt.colormaps():  # check if it's a recognized matplotlib colormap
            cmap = plt.get_cmap(self.c_map)

        # Default to a specific colormap if no match is found
        else:
            cmap = LinearSegmentedColormap.from_list("RedBlack", red_)  # default colormap
