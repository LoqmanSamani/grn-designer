...
class ScatterAnimation:
    ...
    def generate_frame(self, i):
        ...
        for j, z_data in enumerate(self.z_datas):
            ...
            scatter = ax.scatter(self.X.flatten(), self.Y.flatten(), z_data[:, :, i].flatten(),
                                 c=z_data[:, :, i].flatten(), cmap=self.cmap_(j))
            ...
            if self.colorbar:
                cbar = fig.colorbar(scatter, ax=ax, fraction=0.046, pad=self.pad)
