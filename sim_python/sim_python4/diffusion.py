...
@jit
def diffusion2(specie, length, width, k_diff, dt, compartment_length, compartment_width):
    """
    Args:
    - specie: specie concentration array
    - length: index of the ith cell in compartment(column), integer
    ...
    if length == 0 and width == 0:
        specie[length, width] = lower_left_corner_diff_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
        ...
        specie[length, width] = lower_right_corner_diff_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
        ...
        specie[length, width] = upper_left_corner_diff_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
        ...
        specie[length, width] = upper_right_corner_diff_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == 0 and length != 0 and length != 1 and length != compartment_length-1 and length != compartment_length-2:
        specie[length, width] = lower_side_diff1_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == 0 and length == 1:
        specie[length, width] = lower_side_diff2_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == 0 and length == compartment_length-2:
        specie[length, width] = lower_side_diff3_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == 0 and width != 0 and width != 1 and width != compartment_width-1 and width != compartment_width-2:
        specie[length, width] = left_side_diff1_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == 0 and width == 1:
        specie[length, width] = left_side_diff2_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == 0 and width == compartment_width-2:
        specie[length, width] = left_side_diff3_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == compartment_length-1 and width != 0 and width != 1 and width != compartment_width-1 and width != compartment_width-2:
        specie[length, width] = right_side_diff1_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == compartment_length-1 and width == 1:
        specie[length, width] = right_side_diff2_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif length == compartment_length-1 and width == compartment_width-2:
        specie[length, width] = right_side_diff3_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == compartment_width-1 and length != 0 and length != 1 and length != compartment_length-1 and length != compartment_length-2:
        specie[length, width] = upper_side_diff1_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == compartment_width-1 and length == 1:
        specie[length, width] = upper_side_diff2_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
    ...
    elif width == compartment_width-1 and length == compartment_length-2:
        specie[length, width] = upper_side_diff3_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
        ...
        specie[length, width] = central_diffusion_(
            specie=specie,
            length=length,
            depth=width,
            k_diff=k_diff,
...
@jit
def lower_side_diff1_(specie, length, depth, k_diff, dt):
...
@jit
def lower_side_diff2_(specie, length, depth, k_diff, dt):
...
@jit
def lower_side_diff3_(specie, length, depth, k_diff, dt):
...
@jit
def left_side_diff1_(specie, length, depth, k_diff, dt):
...
@jit
def left_side_diff2_(specie, length, depth, k_diff, dt):
...
@jit
def left_side_diff3_(specie, length, depth, k_diff, dt):
...
@jit
def upper_side_diff1_(specie, length, depth, k_diff, dt):
...
@jit
def upper_side_diff2_(specie, length, depth, k_diff, dt):
...
@jit
def upper_side_diff3_(specie, length, depth, k_diff, dt):
...
@jit
def right_side_diff1_(specie, length, depth, k_diff, dt):
...
@jit
def right_side_diff2_(specie, length, depth, k_diff, dt):
...
@jit
def right_side_diff3_(specie, length, depth, k_diff, dt):
...
@jit
def central_diffusion_(specie, length, depth, k_diff, dt):
...
@jit
def lower_left_corner_diff_(specie, length, depth, k_diff, dt):
...
@jit
def lower_right_corner_diff_(specie, length, depth, k_diff, dt):
...
@jit
def upper_left_corner_diff_(specie, length, depth, k_diff, dt):
...
@jit
def upper_right_corner_diff_(specie, length, depth, k_diff, dt):
