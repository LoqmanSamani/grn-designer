...
def apply_component_production(agent, num_species, time_step, column, species_index):
    ...
    initial_concentration = agent[species_index, :, column].astype(np.float32)
    production_pattern = agent[species_index + 1, :, column].astype(np.float32)
    ...
    for k in range(0, num_species * 2, 2):
        t = agent[-1, k + 1, :int(agent[-1, k, -1])].astype(np.float32)
        if species_index in t:
            idx = np.where(t == species_index)[0][0]
            effect_type = int(agent[-1, k + 1, -int(idx + 1)])
            species_effect = agent[k, :, column].astype(np.float32)
            params = agent[-1, species_index, 3 + idx * 3: 6 + idx * 3].astype(np.float32)
...
def apply_hill_effect(species_1, species_2, rate, hill_coefficient, dissociation_constant, time_step, effect_type):
    ...
    species_1 = species_1.astype(np.float32)
    species_2 = species_2.astype(np.float32)
    rate = np.float32(rate)
    hill_coefficient = np.float32(hill_coefficient)
    dissociation_constant = np.float32(dissociation_constant)
...
def apply_component_production(agent, num_species, time_step, column, species_index):
    ...
    production_pattern = agent[species_index+1, :, column]
    ...
    for k in range(0, num_species * 2, 2):
        ...
        if species_index in t:
            ...
            params = agent[-1, species_index, 3 + idx * 3: 6 + idx * 3]
