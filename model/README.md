...
After searching and optimizing for solutions of the problem (simulation system) using the metaheuristic algorithm (genetic algorithm based on natural selection theory (GABONST)), to tune the solutions which are in the list of the best solutions for the model (simulation system), if it is needed (for example the evolutionary algorithm may not find the exact solution), a gradient based optimization algorithm is defined. 
the algorithm is implemented in form of a class named as:  [`GradientOptimization`](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/gradient/optimization.py). the class implemented with [`TensorFlow`](https://www.tensorflow.org/), which is a popular machine learning library. the reason why it is implemented in tensorflow, is the [`GradientTape`](https://www.tensorflow.org/api_docs/python/tf/GradientTape) module which is a powerful tool for automatic differentiation, which is one of the most (if not the most) important part of any optimization based on gradient. to be able to use gradient tape, the simulation model has to be implemented using tensorflow functionalities, as i mentioned before in this report the ***simulation models***(both: [`individual_simulation(individual)`](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/sim/sim_ind/simulation.py) and [`population_simulation(population)`](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/sim/sim_pop/simulation.py)) are implemented using numpy functionalities. 
To be able to use gradient tape, we reimplemented individual simulation using tensorflow. to be able to distinguish between individual simulation using numpy and the new implementation using tensorflow, the main function is named as [tensor_simulation(...)](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/sim/sim_tensor/tensor_simulation.py) which obviously simulate the system in the exact way as numpy implemented version, but just using tensorflow functionalities.
as was explained in the simulation algorithm part of this report, during each epoch of simulation each compartment is updated using a for loop which updates the compartments columnwise, which means each through each simulation process compartments will be updated in place hunderd or thousand times, which makes numpy array suitable for such tasks, because of the ability of in place updating numpy arrays, which make the implementation easier than tensorflow.
in the case of tensorflow tensors ([`tf.Tensor`](https://www.tensorflow.org/api_docs/python/tf/Tensor)) unfortunately it is not possible to update the tensors in place, because they are immutable! (the reason for immutability of tensors in tensorflow is probably automatic differentiation!). to solve this problem we used [`tf.tensor_scatter_nd_update`](https://www.tensorflow.org/api_docs/python/tf/tensor_scatter_nd_update) which creates a new tensor by applying sparse updates to the input tensor, this means in each update (columnwise update) a new tensor will be created and the information form the previous tensor will be combined with the new information in the new created tensor.


### [GradientTape](https://www.tensorflow.org/api_docs/python/tf/GradientTape)

in machine learning area and in the concept of optimization based on gradients, in each model there is a forward pass and a backward pass. Forward pass consists of passing the data (input data) through the model, for example in a neural network it means pass the data through the layers of the model from the first to the last layer of the model, the output of this process called y_hat or prediction and then using this prediction and the target (y), here we talk about supervise learning in which for each x (input data) there is a y (target), the cost will be calculated, in the simplest case, this is the difference between this two value which is called ***cost***.
in backward pass the system will move backward from the last operation, which in this case is the computation of the cost, to the early operation in the system, and during this movement using chain role in calculus the derivative of each step will be calculated till the derivative of the parameeters of the model is calculated, which are defiend as gradients then this gradient will be used to update the parameters in a direction which reduces the cost, so the objective of backward pass is to reduce the cost which directly leads to increase the accuracy of the model.
in our case forward pass consist of simulating the model with the condidate individual (`y_hat = tensor_simulation(individual, parameters, num_species, num_pairs, stop, time_step, max_epoch`)) and then compute the cost, which is the difference between y_hat(output of the simulation) and y (target pattern), in this algorithm mean squared error (`cost = tf.reduce_mean(tf.square(y_hat - target))`)is used as cost method to compute the difference between the predicted and the target. After forward pass now is time to `GradientTape` to go through the model backward and calculate the gradient of each variable using chain role till the gradient of the parameters are calculated. to reach this automatically tensorflow will create a ***computational graph*** to go through it backward and gradients could be calculated, ***Figure 11*** shows a schematic representation of a possible computational graph in our model for an individual with two species and one complex.


![del](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/figures/com_graph.jpg)

*Figure 11: Computational Graph*
