...
The backward pass involves calculating gradients using the chain rule of calculus. Starting from the cost computation, TensorFlow’s `GradientTape` moves backward through the model, computing the derivatives of the cost with respect to each parameter. These gradients are then used to update the model parameters in a direction that reduces the cost, thereby improving the model's accuracy.

In our case, the forward pass involves simulating the model with the candidate individual (`y_hat = tensor_simulation(individual, parameters, num_species, num_pairs, stop, time_step, max_epoch)`) and computing the cost, defined as the mean squared error (`cost = tf.reduce_mean(tf.square(y_hat - target))`). After the forward pass, `GradientTape` computes the gradients of the parameters by traversing the computational graph backward. TensorFlow constructs this computational graph to facilitate automatic differentiation.

The computed gradients are then used to update the model parameters, reducing the cost. We employ the [Adam Algorithm](https://arxiv.org/abs/1412.6980) (`optimizer = tf.keras.optimizers.Adam(learning_rate, weight_decay)`), a robust gradient-based optimization algorithm, to perform these updates effectively. 
...
![Computational Graph](https://github.com/LoqmanSamani/master_project/blob/systembiology/model/figures/com_graph.jpg)
